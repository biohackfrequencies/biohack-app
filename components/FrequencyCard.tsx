...
export const FrequencyCard: React.FC<FrequencyCardProps> = ({ frequency, onSelect, isFavorite, onToggleFavorite }) => {
  ...
  const cardStyle = {
    ...
    minHeight: '224px' // Consistent height for cards
        ...
        >
          ...
          <div className="relative z-10">
            <div className="flex justify-between items-start">
                <h3 className="text-xl font-display font-bold text-white tracking-wide drop-shadow-md pr-4">{frequency.name}</h3>
                {isLocked && <ProBadge />}
            </div>
            <p className="text-sm mt-1" style={{color: frequency.colors.accent}}>{frequency.range}</p>
            <p className="mt-3 text-white/90 text-sm line-clamp-3 group-hover:line-clamp-none drop-shadow-sm">{frequency.description}</p>
          </div>
        </button>
        <button 
